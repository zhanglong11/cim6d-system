...
export default {
  ...
  methods: {
    ...
    setPrincipalByUserId(row) {
      this.axios.system.get(`/cim6d/system/department/admin/add/${row.departmentId}/${this.form.id}`).then(res => {
        this.refresh()
      })
    },
    cancelPrincipalByUserId(row) {
      this.axios.system.get(`/cim6d/system/department/admin/delete/${row.departmentId}/${this.form.id}`).then(res => {
        this.refresh()
      })
    }
  }
