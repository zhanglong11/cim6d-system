...
export default {
  ...
  created() {
    this.axios.system.get('/cim6d/system/user/getCompanyAdmin/' + localStorage.getItem('companyId')).then(res => {
  ...
  methods: {
    async submitOld() {
      this.oldUuid = await this.axios
        .global(`cim6d/verify/code/checkSmsCode/${this.oldUuid}/${this.formOld.msgCode}`)
        .then(res => res.data)
      this.step = 2
      this.seconds = null
    ...
    async getMsgCode(type) {
      // type  1 管理员  2 新管理员
      if (type === 1) {
        this.oldUuid = await this.axios.global
          .post('/cim6d/verify/code/sendSmsCode/' + this.currentAdminInfo.id)
          .then(res => res.data)
      } else if (type === 2) {
        this.newUuid = await this.axios.global
          .post('/cim6d/verify/code/sendSmsCode/' + this.formNew.newUserId)
          .then(res => res.data)
      }
