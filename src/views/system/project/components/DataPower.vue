<template>
  <div ref="tabInnerContainer" class="tabInnerContainer">
    <div class="aa">
      <el-card>
        <vxe-table
          ref="xTable"
          ...
          <vxe-table-column field="range" title="数据权限范围" :edit-render="{}" align="left">
            <template v-slot:edit="{ row }">
              <template v-if="row.type === 2">
                <el-radio-group v-model="row.rangeType" @change="e => handleRadioChange(e, row)">
                  <el-radio :label="1">所有单位</el-radio>
                  <el-radio :label="2">本单位</el-radio>
                  <el-radio :label="3">指定单位</el-radio>
                </el-radio-group>
                <span v-if="row.rangeType === 3" class="selectBtn" @click="handleSelectClick(row)"
                  ><i class="el-icon-plus"></i>选择</span
                >
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      window.addEventListener(
        'scroll',
        e => {
          this.scrollTop = this.$refs.tabInnerContainer.scrollTop
        },
        true
      )
    })
  ...
  methods: {
    //获取配置
    async getSetting() {
      let res = await projectApi.getProjectDataPower(this.roleId).catch(e => (this.dataPowerList = []))
      this.dataPowerList = (res && res.data) || []
    },
    async initData() {
    ...
    async addSelect() {
      ...
      this.$refs.tabInnerContainer.scrollTo(0, this.scrollTop || 0)
    },
