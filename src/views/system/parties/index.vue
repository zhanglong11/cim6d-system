<template>
  <div>
    <vxe-table border :span-method="rowspanMethod" :data="tableData">
      ...
      <vxe-table-column field="participantTypeName" title="参与方性质" />
      <vxe-table-column field="participantCompanyName" title="参与方名称" />
      <vxe-table-column field="projectRoleName" title="角色" />
      <vxe-table-column field="participantUserName" title="参与方人员" />
      <vxe-table-column field="participantTypeName" title="操作">
...
export default {
  ...
  methods: {
    ...
    rowspanMethod({ row, $rowIndex, column, data, columnIndex, $columnIndex }) {
      const fields = ['projectName', 'participantTypeName', 'participantCompanyName']
      ...
      if ($columnIndex === 0) {
        ...
        if (cellValue && fields.includes(column.property)) {
          ...
          if (
            ...
            prevColPrevRowCellValue === prevColCellValue
          ) {
            return { rowspan: 0, colspan: 0 }
          } else {
            let countRowspan = 1
            while (
              ...
              prevColNextRowCellValue === prevColCellValue
            ) {
              nextRow = data[++countRowspan + $rowIndex]
            }
            if (countRowspan > 1) {
